...
txs_needing_more_guides = data.table(fread("coding_exons/txs_needing_more_guides.csv", sep=" ", header=TRUE))

### get the ENSEMBL transcript codes for the txs with low (or no) coverage
mouse = useMart("ensembl", dataset = "mmusculus_gene_ensembl")
txs = as.character(unlist(txs_needing_more_guides[!duplicated(transcript),transcript]))
